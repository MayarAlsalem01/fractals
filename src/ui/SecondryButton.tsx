...
export default function SecondryButton({ children, type = 'button', onClick, className, disabled }: { children: ReactNode, type?: 'button' | 'submit', onClick?: () => void, className?: string, disabled?: boolean }) {
    return (
        <button disabled={disabled} type={type} onClick={onClick} className={`sec-btn  px-3 py-1 border border-accent-foreground/30 rounded-br-2xl rounded-tl-2xl  relative overflow-hidden whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed ${className}`}>
            {children}
