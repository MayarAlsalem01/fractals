...
export default function SecondryButton({ children, type = 'button', onClick }: { children: ReactNode, type?: 'button' | 'submit', onClick?: () => void, }) {
    return (
        <button type={type} onClick={onClick} className="sec-btn w-full px-3 py-1 border border-accent-foreground/30 rounded-br-2xl rounded-tl-2xl  relative overflow-hidden whitespace-nowrap">
            {children}
